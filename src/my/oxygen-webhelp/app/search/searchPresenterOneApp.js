define(['dom-sanitizer', 'localization', 'options', 'util', 'jquery', 'jquery.highlight', 'jquery.bootpag'], function(domSanitizer, i18n, options, util, $) {
    ...
    function addPaginationNav(meta, onPageChangeHandler) {
		$("#wh-search-pagination").html("");
        if (meta.totalPages > 1) {
			var strNext=i18n.getLocalization("next.page");
			var strPrev=i18n.getLocalization("prev.page");
			var pUl = $("#wh-search-pagination").append("<ul class='pagination ' style='border:none'/>"); 
			var pLi = $("<li class='prev '/>").appendTo(pUl);
			var infoLi = $("<li/>").appendTo(pUl);
			var nLi = $("<li  class='next '/>").appendTo(pUl);
			$(pLi).html(strPrev);
			$(nLi).html(strNext);
			
			if( meta.currentPage == 1 ) {
				$(pLi).addClass("disable_link");
			}else{
				$(pLi).on("click", function(){
					//$("#iptPageNum").val(meta.currentPage -1);
					movePageNum( meta.currentPage, meta.currentPage -1, onPageChangeHandler, meta.originalSearchExpression);
				});
			}
			if( meta.currentPage == meta.totalPages ) {
				$(nLi).addClass("disable_link");
			}else{
			...
			$(infoLi).html( "<font>"+meta.currentPage+"</font><font>" + " / " +"</font><font>"+meta.totalPages+"</font>");

			
			/*
			<input type='text' id='iptPageNum' value='"+meta.currentPage+"'/>
			$("#iptPageNum").on("change", function(){
				movePageNum( meta.currentPage, $(this).val(), onPageChangeHandler, meta.originalSearchExpression);
			});  */
        ...
        $('#wh-search-pagination').find('li').addClass('page-link');

        $("#search").trigger('click');
    }

	function movePageNum( oldPage,  num , onPageChangeHandler, originalSearchExpression){
		   var oldQuery = window.location.search;
                ...
                onPageChangeHandler(num, originalSearchExpression);
	}


    function addPaginationNum(meta, onPageChangeHandler) {
        if (meta.totalPages > 1) {
    ...
    function addPagination(meta, onPageChangeHandler) {
		...
		addPaginationNav(meta, onPageChangeHandler);
    }
